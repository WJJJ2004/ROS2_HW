...
void detectAndLabel(const cv::Mat& image, const cv::Mat& mask,cv::Scalar boxColor) 
{
...
int main()
{
    ...
    // 객체 검출 및 레이블링
    detectAndLabel(image, redMask, cv::Scalar(0, 0, 255));    // 빨간색 풍선
    detectAndLabel(image, greenMask, cv::Scalar(0, 255, 0)); // 초록색 풍선
    detectAndLabel(image, blueMask,  cv::Scalar(255, 0, 0));   // 파란색 풍선
